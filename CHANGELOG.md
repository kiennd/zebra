...
## [Zebra 7.0.0](https://github.com/ZcashFoundation/zebra/releases/tag/v7.0.0) - 2026-09-08
...
- Explorer nodes now use state database format 31 for complete Turnstile lifecycle analytics and exact historical mining intervals. Existing format-30 state is not reused and must be synced again.
- Explorer state format 31 includes complete Ironwood Upgrade Observatory history. Existing explorer state must be synced again because transaction-level migration predicates cannot be reconstructed by the bounded online migration.
- Explorer nodes now use state database format 32 for corrected Turnstile chain-time semantics and bounded unspent-marker storage. Existing format-31 explorer state must be synced again.
